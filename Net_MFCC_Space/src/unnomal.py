...
import numpy as np
from load_unnormal import read_features, read_file_test
import math
import os,re
import logging
...
class Deep_Neural_Network():
    ...
    def _load_parameters(self):   
        ...
        self.id_file = 1
        self.weight_folder = '../weight_unnormal/100ms/' + self.hidden_layer + self.artic + 'test_' + str(self.test_number) + '/'
    ...
    def _testing_noise_space(self, test_dir, type_data):             
        for type_test in sorted(os.listdir(test_dir)):
            if (not type_test.endswith('zip')) and 'output' not in type_test:                
                ...
                print type_test_dir
                dnn_predict_dir = '/home/danglab/3P/Net_Space/unnormal/' + self.hidden_layer +'100ms/' + self.artic + type_data + 'test_' + str(self.test_number) + '/' + type_test + '/'
    ...
    def _testing_original_file(self, test_dir, type_data):     
        dnn_predict_dir = '/home/danglab/3P/Net_Space/unnormal/' + self.hidden_layer +'100ms/' + self.artic + type_data + 'test_' + str(self.test_number) + '/' 
