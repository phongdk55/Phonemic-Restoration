...
space_audio_dir = '/home/danglab/Phong/features/Space/0_20ms_1_-6dB/'
...
def interpolate_20(position):
    ...
    #return position
    for i in xrange(5,m-2,4):         # 5,9..., gia su trong 20ms chi co 1 vi tri chinh giua bi loi
        #print i
        try:
            #position[i] = (position[i-2] + position[i-1]+ position[i+1] + position[i+2]) / 4           #interpolate by averaging
            position[i] = (position[i-2] + position[i+2]) / 2
            position[i-1] = (position[i-2] + position[i]) / 2
            position[i+1] = (position[i] + position[i+2]) / 2
        except:
            continue 
    #print position[:10,0]
    return position
...
def get_position(filename):
    ...
    position = arr[:,13:13 + 12].copy()        # position at i
...
def normalise_unit(arr):
    #return arr
    
    max_value = abs(arr).max()
    print max_value
...
def processing():
    ...
    for filename in list_file:
        ...
        write_features(space_audio_dir + filename.replace(".txt", "_20ms_in.txt") , pos, vec, acc, artic_inter_dir + filename)      
